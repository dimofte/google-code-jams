...
function computeRepetitionsAndMaximums(n: number): Object {
  ...
  const bigHalfSetResults =
    cachedRepsMaxes[half] || computeRepetitionsAndMaximums(half);
  if (n % 2 === 1) {
  ...
  const smallHalfSetResults =
    cachedRepsMaxes[half - 1] || computeRepetitionsAndMaximums(half - 1);
  const maxes = union(
    Object.keys(bigHalfSetResults),
    Object.keys(smallHalfSetResults)
  );
